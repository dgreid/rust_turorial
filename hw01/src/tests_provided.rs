...
#[test]
fn test_mat_mult_2x2() {
    let mat1 = vec![vec![2., 3.], vec![2., 5.]];
    let mat2 = vec![vec![3., 5.], vec![6., 2.]];
    let matres = vec![vec![24.,16.], vec![36.,20.]];
    let result = mat_mult(&mat1, &mat2);
    println!("result is {} x {}", result.len(), result[0].len());
    for i in 0..result.len() {
        for j in 0..result[i].len() {
            assert_eq!(result[i][j], matres[i][j]);
        }
    }
}
